...
public class ServerMainFrame {
	...
	public ServerMainFrame() {
		...
		// 更改JFrame的图标：
		frame.setIconImage(Toolkit.getDefaultToolkit().createImage(ServerMainFrame.class.getResource("../Image/QQ_64.png")));
		contentArea = new JTextArea();
		contentArea.setEditable(false);
		contentArea.setForeground(Color.blue);
		txt_message = new JTextField();
		txt_max = new JTextField("30");
		txt_port = new JTextField("8081");
		...
		public void run() {
			while (true) {// 不停的等待客户端的链接
				try {
					...
					contentArea.append(client.getUser().getUsername()
			...
			try {
				...
				for (int i = clients.size() - 1; i >= 0; i--) {
					clients.get(i).getWriter().println(
							"ADD@" + user.getUsername() + user.getIp());
					clients.get(i).getWriter().flush();
				}
			} catch (IOException e) {
				e.printStackTrace();
